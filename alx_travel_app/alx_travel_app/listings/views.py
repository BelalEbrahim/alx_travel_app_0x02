...
def initiate_payment(request):
    ...
    Payment.objects.create(booking=booking, amount=amount, tx_ref=tx_ref, checkout_url=url)
...
def verify_payment(request):
    ...
    r = requests.get(f"{BASE}/transaction/verify/{tx_ref}", headers={"Authorization": f"Bearer {KEY}"}, timeout=20)
    j = r.json()
    ok = j.get("status") == "success" and (j.get("data") or {}).get("status") == "success"
